...
namespace Natue\Bundle\CoreBundle\Datatable\Query;
 ...
 */
class DatatableQuery extends \Sg\DatatablesBundle\Datatable\Data\DatatableQuery
{
    /**
     * @var Serializer
     */
        ...
        foreach ($this->columns as $key => $column) {
            ...
            if (true === $this->isSelectColumn($data)) {
                if ($column instanceof AggregateColumn) {
                    $this->aggregateColumns[] = $data;
                    $this->orderColumns[] = null;
                    $this->searchColumns[] = null;

                    if (!$data = $column->getJoinableExpr()) {
                        continue;
                    }
                }
    ...
    private function setSelectFrom()
    {
        ...
        foreach ($this->aggregateColumns as $collumn) {
            $this->qb->addSelect($collumn);
        }
    ...
    /**
     * Searching / Filtering.
     * Construct the HAVING clause for server-side processing SQL query.
     *
     * @param QueryBuilder $qb
     *
     * @return $this
     */
    private function setHaving(QueryBuilder $qb)
    {
        // just for individual filtering
        if (! $this->individualFiltering ) {
            return $this;
        }

        foreach ($this->columns as $key => $column) {

            if (! $this->isSearchColumn($column) || ! $column instanceof AggregateColumn) {
                continue;
            }
            ...
            $qb->andHaving($column->getTitle() . ' = \'' . $searchValue . '\'');
        }
    ...
    private function getCountFilteredResults($rootEntityIdentifier, $buildQuery = true)
    {
        if (true === $buildQuery) {
            ...
            $this
                ...
                ->setFirstResult(null)
                ->setMaxResults(null)
                ->select('count(' . $this->tableName . '.' . $rootEntityIdentifier . ')');
            if (true === $this->isPostgreSQLConnection) {
    ...
    public function getResponse($buildQuery = true)
    {
        ...
        $fresults = $this->getQuery()->getQuery()->getArrayResult();
